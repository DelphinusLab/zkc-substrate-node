...
fn balance_add<T: Config>(
    ...
    let new_amount = BalanceMap::<T>::get((&account, &token_address))
        .checked_add(amount)
...
fn share_add<T: Config>(
    account: &T::AccountId,
    pool_index: &(TokenAddr, TokenAddr),
    amount: Amount,
) -> Result<Amount, Error<T>> {
    let new_amount = ShareMap::<T>::get((&account, &pool_index))
        .checked_add(amount)
...
fn pool_index<T: Config>(
    token_from: &TokenAddr,
    token_to: &TokenAddr,
) -> Result<(TokenAddr, TokenAddr), Error<T>> {
