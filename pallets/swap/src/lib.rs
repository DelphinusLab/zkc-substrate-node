...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        pub fn pool_supply(
            ...
            valid_input_y_amount::<T>(liq0, liq1, amount0, amount1, true).ok_or(Error::<T>::InvalidAmount)?;
        ...
        pub fn pool_retrieve(
            ...
            let (token0, token1, liq0, liq1, _) = PoolMap::get(&pool_index).ok_or(Error::<T>::PoolNotExists)?;
            valid_input_y_amount::<T>(liq0, liq1, amount0, amount1, false).ok_or(Error::<T>::InvalidAmount)?;
