...
#[test]
//A supply, multi-swap, B supply,  B retrieve, A retrieve
fn multi_supplier_multi_swap_retrieve_all_works_swap_not_after_supply() {
    new_test_ext().execute_with(|| {
        ...
        //PoolSupply amount0 500 and amount1 500 for poolIndex 0, caller is accountIndex 2
        origin = 2u64;
        account_index = 2u32;
        let pool_index = 0u32;
        let mut amount0 = U256::from(500);
        let mut amount1 = U256::from(500);
        nonce = 1u64;
        let secret_key_2 = [
        ...
        //Swap amount 5 from tokenIndex0 to tokenIndex1 for poolIndex 0, caller is accountIndex 0, reverse is 0
        origin = 0u64;
        account_index = 0u32;
        let reverse = 0u8;
        amount = U256::from(5);
        ...
        //PoolSupply amount0 500 and amount1 483 for poolIndex 0, caller is accountIndex 3
        //AccountIndex 2 supply after swap
        origin = 3u64;
        account_index = 3u32;
        amount0 = U256::from(500);
        amount1 = U256::from(483);
        nonce = 1u64;
        let secret_key_3 = [
        ...
        //PoolRetrieve amount0 499 and amount1 481 for poolIndex 0, caller is accountIndex 3
        origin = 3u64;
        account_index = 3u32;
        amount0 = U256::from(499);
        amount1 = U256::from(481);
        nonce = 2u64;
        ...
        assert_eq!(PoolMap::get(pool_index).unwrap(), (0u32, 1u32, U256::from(511), U256::from(494), U256::from(500_980_392_156_862_745u128)));

        assert_eq!(BalanceMap::get((&account_index, 0u32)), U256::from(1999));

        assert_eq!(BalanceMap::get((&account_index, 1u32)), U256::from(1998));

        assert_eq!(ShareMap::get((&account_index, &pool_index)), U256::from(980_392_156_862_745u128));

        assert_eq!(NonceMap::<Test>::get(3u64), 3u64);

        //PoolRetrieve amount0 510 and amount1 493 for poolIndex 0, caller is accountIndex 2
        origin = 2u64;
        account_index = 2u32;
        amount0 = U256::from(510);
        amount1 = U256::from(493);
        nonce = 2u64;
        ...
        assert_eq!(PoolMap::get(pool_index).unwrap(), (0u32, 1u32, U256::from(1), U256::from(1), U256::from(980_392_156_862_745u128)));

        assert_eq!(BalanceMap::get((&account_index, 0u32)), U256::from(2010));

        assert_eq!(BalanceMap::get((&account_index, 1u32)), U256::from(1993));
