...
fn prepare_unit_test_without_supply() {
    //SetKey for accountIndex 0
    let mut origin = 0u64;
    let pub_key_0: [u8; 32] = [
        31, 191,  89, 175,  20, 249,  30,  36,
...
#[test]
fn swapped_out_pool_amount_is_zero() {
    new_test_ext().execute_with(|| {
        prepare_unit_test_without_supply();
        
        //Swap amount 1 from an empty pool, caller is accountIndex 2
        let origin = 2u64;
        let account_index = 2u32;
        let pool_index = 0u32;
        let reverse = 0u8;
        ...
        assert_noop!(SwapModule::swap(Origin::signed(origin), command_sign_formatted, pool_index, reverse, amount, nonce), Error::<Test>::PoolBalanceNotEnough);
    })
}
