...
#[test]
fn calculate_amount1_to_pool_works_is_supply_rem_is_zero() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(calculate_amount1_to_pool::<Test>(&pool_index, amount0, is_supply));

        assert_eq!(calculate_amount1_to_pool::<Test>(&pool_index, amount0, is_supply).unwrap(), U256::from(1500));
    })
}

#[test]
fn calculate_amount1_to_pool_works_is_supply_rem_is_not_zero() {
    new_test_ext().execute_with(|| {
        //PoolMap insert new value
        let pool_index = 0u32;
        let token_index0 = 0u32;
        let token_index1 = 1u32;
        let mut amount0 = U256::from(2000);
        ...
        assert_eq!(calculate_amount1_to_pool::<Test>(&pool_index, amount0, is_supply).unwrap(), U256::from(1521));
    })
}

#[test]
fn calculate_amount1_to_pool_works_is_not_supply() {
    new_test_ext().execute_with(|| {
        prepare_unit_test();
...
fn calculate_amount1_to_pool_pool_not_exists() {
    new_test_ext().execute_with(|| {
        ...
        match calculate_amount1_to_pool::<Test>(&pool_index, amount0, is_supply) {
            Ok(_) => assert!(false),
            Err(e) => {
