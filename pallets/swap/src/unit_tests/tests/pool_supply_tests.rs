...
fn pool_supply_works_share_is_zero() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(PoolMap::get(pool_index).unwrap(), (0u32, 1u32, U256::from(1000), U256::from(1000), U256::exp10(ORDER_OF_MAGNITUDE)));

        assert_eq!(BalanceMap::get((account_index, 0u32)), U256::from(1000));

        assert_eq!(BalanceMap::get((account_index, 1u32)), U256::from(1000));

        assert_eq!(ShareMap::get((&account_index, &pool_index)), U256::from(1_999_999_999_998_000u64));
...
fn pool_supply_works_share_is_not_zero() {
    new_test_ext().execute_with(|| {
        ...
        let account_index = 2u32;
        let pool_index = 0u32;
        ShareMap::insert((&account_index, &pool_index), U256::from(1_999_999_999_998_000u64));
        ...
        assert_eq!(ShareMap::get((&account_index, &pool_index)), U256::from(3_999_999_999_996_000u64));
