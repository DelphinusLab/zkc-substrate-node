#!/usr/bin/env bash
{ read ADMINS; read ENABLEDSTRING; read -r AUTHORITIES_SEEDS; read -r PRE_FUNDED_SEEDS; read -r SUDO_ACCOUNT_SEED; } <<< `node ../deployment/config/get-account-config.js`
echo "ADMINS: " $ADMINS;
echo " "
echo "ENABLEDSTRING: " $ENABLEDSTRING;
echo " "
...
sed -i "s/let nack.*$/$ENABLEDSTRING/" pallets/swap/src/ack_helper.rs
...
echo " "
echo "ack_helper.rs: "
echo "$(<pallets/swap/src/ack_helper.rs)"
echo " "
